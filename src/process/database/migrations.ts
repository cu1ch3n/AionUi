...
/**
 * Migration v15: Add 'discord' to conversations.source CHECK constraint
 */
const migration_v15: IMigration = {
  version: 15,
  name: 'add-discord-source',
  up: (db) => {
    ...
    db.exec(`
      CREATE TABLE IF NOT EXISTS conversations_new (
        ...
        model TEXT,
        status TEXT CHECK(status IN ('pending', 'running', 'finished')),
        source TEXT CHECK(source IS NULL OR source IN ('aionui', 'telegram', 'lark', 'dingtalk', 'discord')),
        channel_chat_id TEXT,
        created_at INTEGER NOT NULL,
        updated_at INTEGER NOT NULL,
        FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE
    ...
    console.log('[Migration v15] Added discord to conversations source constraint');
  },
  down: (db) => {
    db.exec(`
    ...
    console.log('[Migration v15] Rolled back: Removed discord from source constraint');
  },
};
